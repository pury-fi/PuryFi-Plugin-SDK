...
connection.once("open", async ({ version, apiVersion }) => {
   ...
   connection.once("message", "ready", async () => {
      ...
      /**
       * get WBlist configuration as an example of sending a message to PuryFi and awaiting a response.
       */
      let data = await connection.sendMessage("getState", {path: "wblistConfiguration"});
      console.log("Received wblistConfiguration:", data);
   });
});
