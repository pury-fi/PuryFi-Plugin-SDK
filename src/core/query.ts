...
type ReadOnly<T, A extends AccessFor<T>> =
   ...
   : A["access"] extends "noRead" | "noReadWrite" ? never
   : T;
...
type WriteOnly<T, A extends AccessFor<T>> =
   ...
   : A["access"] extends "noWrite" | "noReadWrite" ? never
   : T;
...
type ValueOfAccess<T extends AccessFor<object>, P extends string> =
   P extends `${infer K}.${infer Rest}` ?
      K extends keyof T["entries"] ?
         ...
         :  never
      :  never
   : P extends keyof T["entries"] ? T["entries"][P]
   : never;
...
type HasReadAccess<T, A extends AccessFor<T>> =
   A["access"] extends "noRead" | "noReadWrite" ? false : true;

type HasWriteAccess<T, A extends AccessFor<T>> =
   A["access"] extends "noWrite" | "noReadWrite" ? false : true;
